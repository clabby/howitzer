    ...
    V: Clone + Eq + PartialEq + Encodable + Decodable,
{
    ...
    /// is left as-is.
    pub fn blind(&mut self, cache: &mut FxHashMap<B256, TrieNode<V>>) {
        if self.length() >= B256::ZERO.len() && !matches!(self, TrieNode::Blinded { .. }) {
            ...
            cache.insert(commitment, self.clone());
            *self = TrieNode::Blinded { commitment }
        }
    }

    /// Unblinds the [TrieNode] if it is a [TrieNode::Blinded] node.
    pub fn unblind(&mut self, cache: &FxHashMap<B256, TrieNode<V>>) -> Result<()> {
        if let TrieNode::Blinded { commitment } = self {
            if *commitment == EMPTY_ROOT_HASH {
                ...
                *self = cache.get(commitment).ok_or_else(|| anyhow!("Missing preimage"))?.clone();
            }
        }
        Ok(())
    }
    ...
    pub fn open<'a>(
        &'a mut self,
        path: &Nibbles,
        cache: &FxHashMap<B256, TrieNode<V>>,
        ...
        match self {
            TrieNode::Branch { ref mut stack } => {
                ...
                stack
                    ...
                    .map(|node| node.open(&path.slice(BRANCH_NODE_NIBBLES..), cache))
                    .unwrap_or(Ok(None))
            }
            ...
            TrieNode::Extension { prefix, node } => {
                if path.slice(..prefix.len()).as_slice() == prefix.as_slice() {
                    ...
                    node.unblind(cache)?;
                    node.open(&path.slice(prefix.len()..), cache)
                } else {
                    Ok(None)
                }
            }
        ...
        match self {
            ...
            TrieNode::Extension { prefix, node } => {
                ...
                if shared_extension_nibbles == prefix.len() {
                    node.insert(&path.slice(shared_extension_nibbles..), value, cache)?;
                    return Ok(());
                }
            ...
            TrieNode::Branch { stack } => {
                ...
                let branch_nibble = path[0] as usize;
                stack[branch_nibble].insert(&path.slice(BRANCH_NODE_NIBBLES..), value, cache)
            }
            TrieNode::Blinded { .. } => {
                // If a blinded node is approached, reveal the node and continue the insertion
                // recursion.
                self.unblind(cache)?;
                self.insert(path, value, cache)
            }
        }
    }
    ...
    pub fn delete(&mut self, path: &Nibbles, cache: &FxHashMap<B256, TrieNode<V>>) -> Result<()> {
        match self {
            TrieNode::Empty => {
            ...
            TrieNode::Extension { prefix, node } => {
                ...
                node.delete(&path.slice(prefix.len()..), cache)?;
            ...
            TrieNode::Branch { stack } => {
                let branch_nibble = path[0] as usize;
                stack[branch_nibble].delete(&path.slice(BRANCH_NODE_NIBBLES..), cache)?;
    ...
    pub fn encode_in_place(
        ...
        match self {
            ...
            Self::Extension { prefix, node } => {
                ...
                node.blind(cache);
                node.encode_in_place(out, cache);
            }
            Self::Branch { stack } => {
    ...
    fn collapse_if_possible(&mut self, cache: &FxHashMap<B256, TrieNode<V>>) -> Result<()> {
        match self {
    ...
    fn encode(&self, out: &mut dyn alloy_rlp::BufMut) {
        match self {
            ...
            Self::Extension { prefix, node } => {
                ...
                blinded.blind(&mut Default::default());
                blinded.encode(out);
            }
            Self::Branch { stack } => {
                // In branch nodes, if an element is longer than 32 bytes in length, it is blinded.
...
mod test {
    ...
    use alloy_rlp::{BufMut, Decodable, Encodable, EMPTY_STRING_CODE};
    ...
    fn test_decode_branch() {
        ...
        let expected = TrieNode::Branch {
            stack: vec![
                TrieNode::<Bytes>::new_blinded(b256!(
    ...
    fn test_encode_decode_extension_blinded_long() {
        ...
        let opened = TrieNode::<Bytes>::Leaf {
            prefix: Nibbles::from_nibbles([0x00]),
            value: [0xFF; 64].into(),
        };
        opened.encode(&mut rlp_buf);
        let blinded = TrieNode::<Bytes>::new_blinded(keccak256(&rlp_buf));
    ...
    fn test_retrieve_from_trie_simple() {
        ...
        let mut trie = ordered_trie_with_encoder(&VALUES, |v, buf| {
            ...
            TrieNode::<Bytes>::new_blinded(keccak256(encoded_value)).encode(buf);
        });
        let root = trie.root();

        let mut preimages =
            trie.take_proofs().into_iter().fold(FxHashMap::default(), |mut acc, (_, value)| {
                let node = TrieNode::<Bytes>::decode(&mut value.as_ref()).unwrap();
                acc.insert(keccak256(value.as_ref()), node);
                acc
            });

        let mut root_node = preimages.get(&root).unwrap().clone();
        for (i, value) in VALUES.iter().enumerate() {
            let path_nibbles = Nibbles::unpack([if i == 0 { EMPTY_STRING_CODE } else { i as u8 }]);
            let v = root_node.open(&path_nibbles, &preimages).unwrap().unwrap();

            let mut encoded_value = Vec::with_capacity(value.length());
            value.encode(&mut encoded_value);
            let mut encoded_node = Vec::new();
            TrieNode::<Bytes>::new_blinded(keccak256(&encoded_value)).encode(&mut encoded_node);

            assert_eq!(v, encoded_node.as_slice());
        }

        root_node.blind(&mut preimages);
    ...
    fn test_insert_static() {
        ...
        node.insert(&Nibbles::unpack(hex!("012345")), bytes!("01"), &Default::default()).unwrap();
        node.insert(&Nibbles::unpack(hex!("012346")), bytes!("02"), &Default::default()).unwrap();
    ...
    proptest::proptest! {
        ...
        fn diff_hash_builder_insert(mut keys in proptest::collection::vec(proptest::prelude::any::<[u8; 32]>(), 1..4096)) {
            ...
            let mut node = TrieNode::<Bytes>::Empty;

            for key in keys {
                hb.add_leaf(Nibbles::unpack(key), key.as_ref());
                node.insert(&Nibbles::unpack(key), key.into(), &Default::default()).unwrap();
            }

            node.blind(&mut Default::default());
        ...
        fn diff_hash_builder_delete(mut keys in proptest::collection::vec(proptest::prelude::any::<[u8; 32]>(), 1..4096)) {
            ...
            for deleted_key in deleted_keys {
                node.delete(&Nibbles::unpack(deleted_key), &Default::default())
                    .unwrap();
            }
